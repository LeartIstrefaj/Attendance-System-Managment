<div class="w-full block mt-8">
    <div class="flex flex-wrap sm:flex-no-wrap justify-between">
        <div class="bg-color w-full sm:max-w-sm bg-gray-200 text-center border border-gray-300 rounded px-8 py-6 my-4 sm:my-0">
            <h3 class="text-gray-700 uppercase font-bold">
                <span class="text-4xl color">{{ sprintf("%02d", $parents->children_count) }}</span>
                <span class="leading-tight color">Children</span>
            </h3>
        </div>
    </div>
</div>
...
<div class="w-full block mt-4 sm:mt-8">
    <div class="flex flex-wrap sm:flex-no-wrap justify-between">
        @foreach ($parents->children as $key => $children)
            <div class="bg-color w-full bg-gray-200 text-center border border-gray-300 rounded px-8 py-6 mb-4 {{ ($key>=1) ? 'ml-0 sm:ml-2' : '' }} {{ ($parents->children_count===1) ? 'sm:max-w-sm' : '' }}">
                <div class="text-gray-300 font-bold">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('attendance.show',$children->id) }}" class="btn-3 bg-gray-100 inline-block mb-4 text-xs color uppercase font-semibold px-4 py-2  rounded">Attendence</a>
